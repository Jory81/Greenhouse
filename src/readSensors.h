  ...
  switch (systemParam.probeTypeT){
          case 1: {
            for (int sensor = 0; sensor <systemParam.probeCountT; sensor++){
              ...
              temp[sensor] = processRTD(rtd) - calSettings.pt100sensor[sensor];              
              ...
              temp[sensor] = totalTemp[sensor]/systemParam.measurements;
              }
    ...
    if (systemParam.probeCountH > 0){
        ...
        predhtTemp[sensor] = dht[sensor].readTemperature();
          if(!isnan(predhtTemp[sensor]) && !isnan(preHumidity[sensor])){
            dhtTemp[sensor] = predhtTemp[sensor] - calSettings.dhtTemp[sensor];
            humidity[sensor] = preHumidity[sensor] - calSettings.dhtHumid[sensor];
